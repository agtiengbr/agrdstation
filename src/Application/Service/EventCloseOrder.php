...
class EventCloseOrder 
{
    ...
    public function getPaymentMethod($payment_method){
            $methodName=self::remove_accents(str_replace(' ','',$payment_method));
            $paymentName=\Configuration::get('AGRDSTATION_MAPPING_PAYMENT_'.$methodName);
            return $paymentName;
    }

    public function exec($order,$lang_id)
    {
        ...
        $bodyRequest=[
            ...
            'payload'=>[
                ...
                'cf_order_status'=>$order->getCurrentStateFull($lang_id)['name'],
                'cf_order_payment_method'=>$this->getPaymentMethod($order->payment),
                'cf_order_payment_amount'=>(Float)$order->getTotalProductsWithTaxes()/1,
                "legal_bases"=> [
                    [
